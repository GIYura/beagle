...
/* statically defined spinlock.
Create a variable named pcd_lock of type struct spinlock_t and spinlock
initialized in unlock state. */
#if 0
...
static ssize_t pcd_read(struct file *filp, char __user *buff, size_t count, loff_t *f_pos)
{
    ...
    /* необходима для разблокировки процесса, который находится в спящем режиме */
